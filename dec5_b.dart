...
int solve(List<String> lines) {
  ...
  final List<({int startSeed, int endSeed})> seedRanges = [];

  for (int i = 0; i < seedMap.seeds.length; i += 2) {
    seedRanges.add(
      (
        startSeed: seedMap.seeds.elementAt(i),
        endSeed: seedMap.seeds.elementAt(i) + seedMap.seeds.elementAt(i + 1),
      ),
    );
  }
  ...
  int location = 0;
  while (true) {
    final seed = seedMap.locationToSeed(location);
